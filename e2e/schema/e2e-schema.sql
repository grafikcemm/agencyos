...
WITH cols AS (
  ...
  WHERE c.relkind = 'r'
    AND c.relname NOT LIKE 'telegram\_%' AND c.relname <> 'active_tasks' AND c.relname <> 'daily_v2'
  GROUP BY c.relname
),
cons AS (
  ...
  WHERE con.contype IN ('p','u','c','f')
    AND rel.relname NOT LIKE 'telegram\_%' AND rel.relname <> 'active_tasks' AND rel.relname <> 'daily_v2'
...
idx AS (
  ...
  WHERE i.schemaname = 'public'
    AND i.tablename NOT LIKE 'telegram\_%' AND i.tablename <> 'active_tasks' AND i.tablename <> 'daily_v2'
...
-- Bu LIFE-mimic DDL'i yorum değil, bootstrap'ın gerçek parçasıdır. Böylece
-- sıfır test projesine e2e-schema.sql uygulandığında Telegram/günlük smoke
-- testleri gizli, daha önce elle uygulanmış migration'lara bağımlı kalmaz.
...
ALTER TABLE public.telegram_outbound_deliveries
  ADD COLUMN IF NOT EXISTS attempt_count integer NOT NULL DEFAULT 1,
...
ALTER TABLE public.daily_v2 ENABLE ROW LEVEL SECURITY;
...
DO $life_e2e_policies$
DECLARE t text;
BEGIN
  FOREACH t IN ARRAY ARRAY[
    'daily_v2', 'active_tasks', 'telegram_update_claims',
    'telegram_pending_actions', 'telegram_outbound_deliveries'
  ] LOOP
    EXECUTE format('DROP POLICY IF EXISTS e2e_open ON public.%I', t);
    EXECUTE format(
      'CREATE POLICY e2e_open ON public.%I FOR ALL TO anon, authenticated, service_role USING (true) WITH CHECK (true)',
      t
    );
...
GRANT EXECUTE ON FUNCTION public.telegram_acquire_update(bigint, integer) TO anon, authenticated, service_role;

...
// ─────────────────────────────────────────────────────────────────────────────
// JARVIS araç-çağıran motoru — hem /api/jarvis (web) hem Telegram webhook'u
// `runJarvis()` ile bu tek kaynağı kullanır. Görevleri GERÇEKTEN yürütür:
// lead taraması, pitch/taslak üretimi, çağrı listesi, görev kaydı (create_task).
// ─────────────────────────────────────────────────────────────────────────────
...
- "mesaj taslağı", "mail taslağı", "soğuk email" (işletme ismi ile) → draft_email
...
- "carousel brief", "içerik fikri", "carousel" → build_carousel_brief
- "görev ekle", "kaydet", "takip et", "araştır ve not al", VEYA bir aracın doğrudan
  karşılamadığı somut bir iş/araştırma görevi → create_task (işi aktif görev olarak kaydet,
  kısaca ilk yön/öneriyi de ver). Araştırma görevlerinde önce create_task çağır, sonra özetle.
...
const JARVIS_TOOLS: JarvisTool[] = [
  ...
  },
  {
    ...
    function: {
      name: 'create_task',
      description: 'Kullanıcının verdiği işi/araştırmayı takip için aktif görev olarak kaydeder (uygulamada ve sabah brifinginde görünür). Bir araç doğrudan karşılamayan somut görevlerde kullan.',
      parameters: {
        type: 'object',
        properties: {
          title: { type: 'string', description: 'Görev başlığı (kısa, net)' },
          category: { type: 'string', enum: ['active', 'waiting'], description: 'active = aktif görev, waiting = bekleyen (varsayılan active)' }
        },
        required: ['title']
      }
    }
...
async function findLeadByNameHelper(name: string): Promise<{ lead: SearchMatchedLead; multipleMatches: string[] | null } | null> {
  ...
  // PostgREST .or() yorumlar virgül/parantez ile bölünür — kullanıcı/LLM kaynaklı
  // parça içindeki bu karakterleri temizle (filter injection guard).
...
Türkçe yaz.`,
    ...
    case 'create_task': {
      ...
      if (!title) return 'Görev başlığı boş olamaz.'
      const category = (args.category as string) === 'waiting' ? 'waiting' : 'active'
      const { error } = await lifeSupabaseAdmin.from('active_tasks').insert({ title, category })
      if (error) return `Görev kaydedilemedi: ${error.message}`
      return `✅ Görev eklendi: "${title}"${category === 'waiting' ? ' (bekleyenler)' : ''}. Aktif görevlerine ve sabah brifingine düşecek.`
    }
...
// --- Public entrypoint (web + Telegram ortak) ---
...
function single(toolName: string, args: Record<string, unknown>, reply: string): JarvisRunResult {
  return { reply, actions: [{ type: toolName, args }], toolCalls: [toolName] }
}

export async function runJarvis(message: string): Promise<JarvisRunResult> {
  ...
  if (isOpportunityStatusMatch) {
    return single('get_opportunity_status', {}, await executeTool('get_opportunity_status', {}))
  }
  ...
  if (isTrendSignalsMatch) {
    return single('get_trend_signals', { limit: 10 }, await executeTool('get_trend_signals', { limit: 10 }))
  }

  if (msgLower.includes('türkiye fırsatı') || msgLower.includes('türkiye açığı') || msgLower.includes('yerel fırsat')) {
    return single('get_turkey_gaps', {}, await executeTool('get_turkey_gaps', {}))
  }
  ...
  if (pitchMatch) {
    ...
    if (targetName && targetName !== 'yaz') {
      return single('generate_call_pitch_by_name', { name: targetName }, await executeTool('generate_call_pitch_by_name', { name: targetName }))
    }
  }

  // 2. Conversion explanation matching
  ...
  if (convMatch) {
    ...
    if (targetName) {
      return single('explain_conversion_by_name', { name: targetName }, await executeTool('explain_conversion_by_name', { name: targetName }))
    }
  }
  ...
  if (analysisMatch) {
    ...
    if (targetName) {
      return single('find_lead_by_name', { name: targetName }, await executeTool('find_lead_by_name', { name: targetName }))
    }
  }

  // --- LLM araç-çağıran akış ---
  ...
  if (toolCalls && toolCalls.length > 0) {
    ...
    if (executedTools.some((name) => directRenderTools.has(name))) {
      return { reply: rawResults.join('\n\n'), actions, toolCalls: executedTools }
    }
    ...
    return { reply: finalReply || toolResults.join('\n'), actions, toolCalls: executedTools }
  }
  ...
  return { reply: content || 'Yanıt üretilemedi — lütfen komutu tekrar gönderin.', actions: [], toolCalls: [] }
}

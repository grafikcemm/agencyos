  ...
  const renderActive = () => {
          ...
          {hasPriorityPending && lockedCount > 0 && (
            <div className="flex items-center gap-2.5 rounded-card border border-dashed border-[var(--border)] px-4 py-3">
              ...
              <p className="text-[11px] text-[var(--text-muted)] leading-relaxed">
                <span className="text-[var(--text-secondary)] font-medium">{lockedCount} görev</span> öncelikli görevler bitince açılır.
              </p>
            </div>
          )}
        </div>
        ...
        {!hideAddInput && (
            <div className="flex gap-2 mt-4">
              <input
                ...
                disabled={isAdding}
                className="flex-1 bg-[var(--bg-surface)] border border-[var(--border-subtle)] rounded-xl px-4 py-2.5 text-sm text-[var(--text-muted)] placeholder-[var(--text-tertiary)] focus:border-[var(--accent)] focus:outline-none transition-all disabled:opacity-50"
              />
              <button
              ...
              {isDev && (
                <button
                  ...
                  disabled={isAdding || !newTaskValue.trim()}
                  className="w-9 h-9 bg-[var(--bg-surface)] border border-[var(--border-subtle)] rounded-xl flex items-center justify-center text-[var(--text-tertiary)] hover:border-[var(--border-strong)] transition-colors disabled:opacity-50"
        ...
        {isDev && waitingOnly.length > 0 && (
          <div className="mt-6 border-t border-[var(--border-subtle)] pt-4">
            <details className="group/waiting-tasks">
              <summary className="flex items-center justify-between text-xs font-semibold tracking-widest uppercase text-[var(--text-tertiary)] hover:text-[var(--text-primary)] transition-colors cursor-pointer select-none py-1.5">
                <span className="flex items-center gap-2">
                  <span>BEKLEYENLER ({waitingOnly.length})</span>
                  <span className="text-[10px] text-[var(--text-tertiary)] font-normal lowercase italic">günlük akışın dışında</span>
                </span>
    ...
    return (
      <details className="group/active-tasks">
        <summary className="flex items-center gap-2 text-xs font-semibold tracking-widest uppercase text-[var(--text-tertiary)] hover:text-[var(--text-primary)] transition-colors cursor-pointer select-none py-2 px-1">
